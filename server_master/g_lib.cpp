...
int listen_master(char addr[])
{
    ...
    strncpy(addr1.sun_path, addr, sizeof(addr1.sun_path) - 1);
    ...
    if( bind(socket_fd, (struct sockaddr *)&addr1, SUN_LEN(&addr1)) == -1)
    {
        fprintf(stderr, "socket bind address error: %s(errno: %d)\n",strerror(errno), errno);
        close(socket_fd);
        exit(1);
    ...
    printf("started GChat worker master server\n");
...
// 接收worker连接
//返回 客户套接字描述符
int accept_worker(int socket_fd)
{
    int worker_fd = 0;
    sockaddr_un worker_addr;
    socklen_t len = sizeof(worker_addr);

    // 接受worker连接
printf("~new worker accept\n");
    if((worker_fd = accept(socket_fd, (struct sockaddr *) &worker_addr, &len) ) == -1)
    {
        if (errno == EINTR)    // 被信号处理中断    重新继续
            return -1;
        else
...
// 参数  #字符串: IP  #字符串: host
// 返回值  套接字文件描述符
int connect_master(char addr[])
{
    int server_fd;
    struct sockaddr_un masaddr;
    ...
    memset(&masaddr, 0, sizeof(masaddr));
    masaddr.sun_family = AF_LOCAL;

    strncpy(masaddr.sun_path, addr, sizeof(masaddr.sun_path) - 1);

    if ( connect(server_fd, (struct sockaddr*) &masaddr, sizeof(masaddr)) == -1)
    {
        close(server_fd);
        err_exit("connect to server error");
    }
